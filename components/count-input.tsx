...
export function CountInput() {
  ...
  return (
    <Card className="p-3 sm:p-4 bg-card/90 backdrop-blur-sm border-primary/20">
      <div className="space-y-3 sm:space-y-4">
        <h3 className="text-foreground font-bold text-center text-sm sm:text-base">Mon Comptage</h3>

        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          <div>
            <label className="text-xs sm:text-sm text-muted-foreground">Running</label>
            <Input
              type="number"
              value={inputRunning}
          ...
          <div>
            <label className="text-xs sm:text-sm text-muted-foreground">True</label>
            <Input
              type="number"
              step="0.5"
              value={inputTrue}
        ...
        <Button onClick={handleSubmit} className="w-full bg-primary hover:bg-primary/90 text-sm sm:text-base">
          Valider
        </Button>

        {(myRunning !== undefined || myTrue !== undefined) && (
          <div className="space-y-2">
            <div className="text-center text-xs sm:text-sm text-muted-foreground">Précision</div>
            <div className="flex flex-col sm:flex-row gap-2 justify-center">
              {myRunning !== undefined && (
                <Badge variant={accuracy.runningCorrect ? "default" : "destructive"} className="text-xs">
